...
# chk Prod site
find $ProdRoot ! \( -perm 700 \) -type d -exec chmod -R 700 {} \;
find $ProdRoot ! \( -user nginx -group nginx \) -type d -exec chown -R nginx:nginx {} \;
find $ProdRoot ! \( -perm 600 \) -type f -exec chmod -R 600 {} \;
find $ProdRoot ! \( -user nginx -group nginx \) -type d -exec chown -R nginx:nginx {} \;
